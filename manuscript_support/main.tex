...
\title{SCAA-PLC: Sample-Level Evidence Contracts for PLC Binary-Analysis Studies}
...
Two further analyses test the reporting method rather than the GEB convention alone. First, a transparent signature decoder is trained on nine inferred-name folds and evaluated on the tenth. All three schemas preserve the 2{,}431-row binary tool-output boundary exactly. For source-function-block presence, the three-channel and six-slot representations have the same balanced accuracy (0.692) and macro F1 (0.638), compared with 0.500 and 0.367 for the two-field summary. The six slots therefore add provenance detail but no measured discrimination over three channels on PLC-BEAD. Second, an adapter instantiates the same contract over an independently generated native OpenPLC/radare2 evidence set~\cite{radare2}. All 10 binary hashes join to 6{,}657 relation rows and 20 uncertainty records. This is a bounded second-pipeline contract instantiation, not evidence of compiler, vendor, or ecosystem portability.
...
Evaluation Cards~\cite{evaluation_cards_arxiv2026} & Joined benchmark, model, and run records; schema derived from 52 papers and 10 stakeholder interviews & Cross-stakeholder evaluation reporting & Human-informed AI-evaluation schema; not binary-tool observations or per-read provenance \\
RARE~\cite{santos_grueiro_rare2026} & Binary-derived observations in LLM-assisted reverse engineering & Authorization, claim support, and provenance under adversarial role promotion & Adversarial authority and support controls; not a PLC sample/run reporting contract \\
...
SCAA-PLC & Per-binary observation, claim-support, uncertainty, and read-provenance records & Source-syntax boundary case, schema reduction, contract invariants, execution audit, and bounded second-pipeline instantiation & Versioned public GitHub release with explicit code/data licenses and a passing clean-clone check; independent taxonomy validation remains outside the evaluated scope \\
\bottomrule
\end{tabularx}
\end{table*}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figs/fig1_scaa_plc_framework_revision_v2.pdf}
...
The evaluation uses the hash-addressed card, uncertainty, source-oracle, contract-invariant, and JSS validation packages. The v6 package adds 2{,}431-row features and cards, a 1{,}876-row source oracle, RQ-specific population flow, fold-wise schema predictions, trace cases, schema coverage, and invariant results. The versioned public GitHub artifact\footnote{\url{https://github.com/Mewtwoz/SCAA_PLC_ARTIFACT/releases/tag/v2.0.0}} supersedes the preliminary commit \texttt{46751eacd629}. It packages the derived inputs selected for verification under package-relative paths, compresses the 195-MiB feature table, records file checksums, and provides a one-command validator plus a clean-room log. A clean clone of release \texttt{v2.0.0} passed \texttt{python3 run\_artifact.py}. Raw PLC-BEAD binaries and sources are not redistributed because the examined upstream snapshot contains no explicit license file; a verifier checks a separately acquired snapshot against the card and source hashes. Author-generated code is released under BSD-3-Clause and author-generated derived data under CC BY 4.0; a persistent archival DOI is recommended.
...
\begin{tabularx}{0.99\textwidth}{>{\raggedright\arraybackslash}p{0.13\textwidth}>{\raggedright\arraybackslash}p{0.22\textwidth}>{\raggedright\arraybackslash}p{0.18\textwidth}>{\raggedright\arraybackslash}p{0.18\textwidth}>{\raggedright\arraybackslash}X}
\toprule
Sample & Raw observation / provenance & EC1--EC6 & Oracle / U-code & Allowed conclusion \\
\midrule
\texttt{geb\_\allowbreak ARRAY\_\allowbreak HAV} & 1,104 \texttt{nm} function symbols; GEB-scoped \texttt{dt\_FB\_}; mapped \texttt{ARRAY\_HAV.ST} & filled / filled / filled / filled / filled / filled & Source FB declaration; no U-code & Lexical rule and source syntax agree for this row; runtime semantics do not follow. \\
\texttt{geb\_\allowbreak AIN} & 1,100 symbols; \texttt{dt\_FN\_AIN}; mapped \texttt{AIN.ST} & filled / filled / filled / partial / filled / filled & Source \texttt{FUNCTION}; U09b & Source-negative contrast supports the GEB naming boundary, not compiler independence. \\
\texttt{codesys\_\allowbreak\_ARRAY\_\allowbreak ABS} & CODESYS \texttt{.app}; configured \texttt{nm} path emits zero symbols & partial / partial / partial / partial / partial / filled & No reliable ST oracle; U01/\allowbreak U02/\allowbreak U11/\allowbreak U13 & The tool path is unsupported for this row; absence of PLC logic is not inferred. \\
\bottomrule
\end{tabularx}
\end{table*}
...
Every source-task signature is ambiguous in the full corpus because the evidence channels do not uniquely identify source POU role. The reported majority decoder is therefore a controlled probe of retained information, not a useful source-role classifier. S3 produces the same held-out decoder outputs as S6 on the two evaluated probes; six slots remain the trace-preserving reporting form.
...
The PLC-BEAD corpus is obtainable from the \href{https://github.com/AICPS/PLCBEAD_PLCEmbed}{PLC-BEAD public repository} (examined commit \texttt{b9593436eb36d1c7847870207692b6b2eb7747c3}). Because that snapshot contains no explicit license file, the SCAA-PLC artifact does not redistribute its raw binaries or sources. It instead provides a hash verifier for a separately acquired copy. The author-generated artifact is publicly accessible as the immutable \href{https://github.com/Mewtwoz/SCAA_PLC_ARTIFACT/releases/tag/v2.0.0}{SCAA-PLC v2.0.0 GitHub release}, which supersedes preliminary commit \texttt{46751eacd629}. A clean clone of the release passed the packaged one-command validator. The release includes manifests, all-population cards, source-oracle outputs, schema predictions, contract-invariant results, trace cases, cross-pipeline derived tables, fixtures, checksums, and environment notes. Code and documentation use BSD-3-Clause; author-generated derived data use CC BY 4.0. No proprietary firmware, vendor artifact, or human-subject result is included.
